...
var RedBox = {
  ...
  {
    this.setWindowWidth(id);
    this.installResize(id);
    this.showOverlay();
  ...
  setOverlaySize: function()
  {
    ...
    $("RB_overlay").style['height'] = dimensions.height +"px";
    $("RB_overlay").style['width'] = dimensions.width +"px";
  },
  ...
  doResize: function(id) {
    this.setWindowWidth(id);
    this.setWindowPosition();
    this.setOverlaySize();
  },

  installResize: function(id)
  {
    Event.observe(document.onresize ? document : window, "resize", this.doResize.bind(this, id));
  },

  setWindowWidth: function(id)
  {
    width = document.viewport.getWidth() * 0.85;
...
var DateFormat = {
  autoParse: function(){
    $$('span.timestamp').each(function(span) {
      ...
      local_time = rel == 'words' ? utc.timeAgoInWords() : utc.strftime(rel);
      span.update(local_time)
    });
  }
}
...
function fullHeightOf(element) {
  var styledHeight = element.style.maxHeight;
  element.style.visibility = 'hidden';
  element.style.maxHeight = '100%';
  var fullHeight = element.offsetHeight;
  element.style.maxHeight = styledHeight;
...
function toggleTruncate(e, element) {
  ...
  if (element.truncated) {
    ...
    element.link.innerHTML = element.link.fullText;

    new Effect.Morph(element,
        { style: { height: element.fullHeight } });
  } else {
    element.truncated = true;
    element.link.innerHTML = element.link.truncatedText;
    new Effect.Morph(element,
        { style: { height: element.truncatedHeight } });
  }
}

function truncate(element) {
  element.truncatedHeight = element.offsetHeight + 'px';
  element.fullHeight = fullHeightOf(element) + 'px';
  if (element.truncatedHeight != element.fullHeight) {
    element.style.maxHeight = '100%';
    element.style.height = element.truncatedHeight;
    element.truncated = true;

    var linkContainer = $(document.createElement('p'));
    linkContainer.className = 'expander';
    element.link = $(document.createElement('a'));
    element.link.truncatedText = 'View details';
    element.link.fullText = 'Hide details';
    element.link.innerHTML = element.link.truncatedText;
    element.link.href = '#';
    element.link.observe('click',
      toggleTruncate.bindAsEventListener(this, element));
    linkContainer.appendChild(element.link);
    element.insert({ after: linkContainer });
  }
}
...
function freePlanSelected() {
  elements = $$(".planradio");
  for(i = 0; i < elements.length; i = i+1) {
    if(elements[i].checked == true && elements[i].hasClassName('free')) {
      return true;
    }
  }
  return false;
...
function setupTabs(dom_id) {
  key = dom_id + '_tabs';
  tabs = new Control.Tabs('subnav', {setClassOnContainer: true, defaultTab: getCookie(key, 'summary')});
  tabs.observe('afterChange', function(new_container) {
    setCookie(key, new_container.readAttribute("id"));
  });
}
